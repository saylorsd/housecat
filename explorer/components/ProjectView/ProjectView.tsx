            ...
            return (
              <section id={key} key={key} className={styles.sectionWrapper}>
                ...
                {recordData.map((record) => (
                  <div key={`${getKey(record)}`} className={styles.itemWrapper}>
...
>(props: TableProps<T>) {
  ...
  return (
    <table className={styles.infoTable}>
      <tbody>
        {schemaEntry.items.map((entry) => {
          const displayValue = formatValue(entry, record);
          return (
            <tr key={getKey(entry)}>
              <td className={styles.fieldCell}>{entry.label}</td>
              <td
...
function formatValue<
  T extends Record<string, ReactNode> = Record<string, ReactNode>,
>({ format, accessor }: SchemaItem<T>, record: T) {
  let value: React.ReactNode | undefined;
  if (typeof accessor === 'function') value = accessor(record);
...
function getKey(record: SchemaItem<any>): string {
  if (typeof record.accessor === 'string') return record.accessor;
  return record.id || '';
}
